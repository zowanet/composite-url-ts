...
export default class URLHash {
	...
	static readonly #fixArgs: [ RegExp, string ] = [ /#/g, '%23' ];
	...
	static #canonicalize(value: string): string {
		return this.#clean(
			Object.assign(
				...
				{
					hash: this.#percentEncoder.encode(value),
				},
			).hash,
		);
	}
	...
	public set value(value: string) {
		const validatedValue: string | undefined = (this.constructor as typeof URLHash).#validate(value);
		if (validatedValue === undefined) {
			throw new Error(`Invalid property URLHash.value: ${ JSON.stringify(value) }; expected valid non-empty hash string`);
		} else {
	...
	public render(): string {
		try {
			return (
				this.#value.length === 0
				? ''
				:  (this.constructor as typeof URLHash).#canonicalize(this.#value)
			);
		} catch (_: unknown) {
			return '';
		}
